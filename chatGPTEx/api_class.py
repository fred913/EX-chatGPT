...
def remove_stopwords(text):
    ...
    zh_filtered_words = [
        word.strip() for word in zh_words
        if word not in zh_stopwords and word != ' '
    ]
...
def clean_string(input_str):
    ...
    pattern = re.compile('|'.join(
        re.escape(key) for key in symbol_dict.keys()))
...
class WikiSearchAPI(MetaAPI):
    ...
    def __init__(
        ...
        super(WikiSearchAPI, self).__init__(api_name=api_name,
                                            base_url=base_url,
                                            api_keys=[''])
    ...
    def call(self, query, num_results=2):
        ...
        data = [
            d['title'] + ": " + remove_html_tags(d["snippet"]) for d in data
        ][:num_results]
        return data
...
class GoogleSearchAPI(MetaAPI):
    ...
    def __init__(
        ...
        super(GoogleSearchAPI, self).__init__(api_name=api_name,
                                              base_url=base_url,
                                              api_keys=apikeys)
    ...
    def call(self, query, num_results=2):
        ...
        try:
            ...
            if "items" in res.json():
                ...
                filter_data = [
                    clean_string(item["title"] + ": " + item["snippet"])
                    for item in items
                ]
                return ('\n').join(filter_data)
            else:
            ...
            if err.response.status_code == 403:  # API key error
                self.trash_api_keys.put(
                    (time.time(), (GOOGLE_API_KEY, GOOGLE_SEARCH_ENGINE_ID)))
...
class WolframAPI(MetaAPI):
    ...
    def __init__(
        ...
        super(WolframAPI, self).__init__(api_name=api_name,
                                         base_url=base_url,
                                         api_keys=apikeys)
    ...
    def call(self, query, num_results=3):
        ...
        try:
            responseFromWolfram = self.session.get(self.base_url,
                                                   params=params,
                                                   headers=headers)
            if 'queryresult' in responseFromWolfram.json(
            ) and 'pods' in responseFromWolfram.json()['queryresult']:
                pods = responseFromWolfram.json(
                )['queryresult']['pods'][:num_results]
