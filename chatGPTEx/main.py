...
def get_bot_response():
    ...
    if mode == "chat":
        ...
        if promptName != "":
            ...
            return Response(directQuery_stream(q, conv_id=uuid, prompt=prompt),
                            direct_passthrough=True,
                            mimetype='application/octet-stream')
        else:
            return Response(directQuery_stream(q, conv_id=uuid),
                            direct_passthrough=True,
                            mimetype='application/octet-stream')
    elif mode == "web":
        q = 'current Time: ' + str(now) + '\n\nQuery:' + str(userText)
        return Response(web(q, conv_id=uuid),
                        direct_passthrough=True,
                        mimetype='application/octet-stream')
    elif mode == "detail":
        q = 'current Time: ' + str(now) + '\n\nQuery:' + str(userText)
        return Response(detail(q, conv_id=uuid),
                        direct_passthrough=True,
                        mimetype='application/octet-stream')
    elif mode == 'webDirect':
        q = 'current Time: ' + str(now) + '\n\nQuery:' + str(userText)
        return Response(webDirect(q, conv_id=uuid),
                        direct_passthrough=True,
                        mimetype='application/octet-stream')
    elif mode == 'WebKeyWord':
        q = str(userText)
        return Response(WebKeyWord(q, conv_id=uuid),
                        direct_passthrough=True,
                        mimetype='application/octet-stream')
    return "Error"
