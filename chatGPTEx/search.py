...
try:
    ...
    exit()
chatbot = ExChatGPT(api_keys=openAIAPIKeys,
                    apiTimeInterval=20,
                    proxy=API_PROXY)
...
max_token = 1000
hint_recall_dialog = json.loads(
    json.dumps(
        {"calls": [{
            "API": "ExChatGPT",
            "query": "Recall our dialogs…"
        }]},
        ensure_ascii=False))
hint_api_finished = json.loads(
    json.dumps({"calls": [{
        "API": "System",
        "query": "API calls finished"
    }]},
               ensure_ascii=False))
...
def detail(query, conv_id='default'):
    ...
    print(f'API calls response:\n {call_res1}')
    result = SumReply(query,
                      str(call_res0) + str(call_res1),
                      max_token=2000,
                      conv_id=conv_id)
    for data in result:
        yield data.encode()
...
def WebKeyWord(query, conv_id='default'):
    ...
    search_res = json.dumps(json.loads(search_results),
                            indent=4,
                            ensure_ascii=False)
...
def APIQuery(query, resp=''):
    with open(program_dir + "/prompts/APIPrompt.txt", "r",
              encoding='utf-8') as f:
        prompt = f.read()
...
def APIExtraQuery(query, callResponse):
    with open(program_dir + "/prompts/APIExtraPrompt.txt",
              "r",
              encoding='utf-8') as f:
        prompt = f.read()
    prompt = prompt.replace("{query}", query)
...
hint_answer_generating = json.loads(
    json.dumps({
        "calls": [{
            "API": "ExChatGPT",
            "query": "Generating answers for you…"
        }]
    }))
...
def SumReply(query, apicalls, max_token=2000, conv_id='default'):
    global APICallList
    APICallList.append(hint_answer_generating)
    with open(program_dir + "/prompts/ReplySum.txt", "r",
              encoding='utf-8') as f:
        prompt = f.read()
...
def Summary(query, callResponse):
    with open(program_dir + "/prompts/summary.txt", "r",
              encoding='utf-8') as f:
        prompt = f.read()
    prompt = prompt.replace("{query}", query)
    ...
    chatbot.reset(
        convo_id='sum',
        system_prompt=
        'Your need to summarize the information you got from the APIs.')
...
def search(content, max_token=2000, max_query=5):
    ...
    call_res = {
        key: value
        for key, value in call_res.items() if len(str(value)) >= 10
    }
